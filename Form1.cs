...
namespace SoilSensorTool
{
    public partial class Form1 : Form
    {
        ...
        private const byte CMD_SET_ONENET = 0x27;
        private const byte CMD_SET_ONENET_REPLY = 0x97;

        private const byte CMD_QUERY_ONENET = 0x28;
        private const byte CMD_QUERY_ONENET_REPLY = 0x98;

        private const byte CMD_UPGRADE_DEVICE = 0x50;
        private const byte CMD_UPGRADE_DEVICE_REPLY = 0xC0;

        private const byte UPGRADE_CMD_ERASE = 0x01;
        private const byte UPGRADE_CMD_WRITE = 0x02;
        private const byte UPGRADE_CMD_END = 0x03;
        private const int UPGRADE_DATA_MAX_LEN = 128;

        private const uint PASSWORD = 0x23F209C3;

        private System.Windows.Forms.Timer timerCurrentTime;

        private byte[] upgradeFileData = null;
        private string upgradeFileName = null;
        private bool isUpgrading = false;
        private BackgroundWorker upgradeWorker = null;
        private BackgroundWorker dataQueryWorker = null;
        private BackgroundWorker allDataQueryWorker = null;
        private Form progressForm = null;
        private ProgressBar queryProgressBar = null;
        private Label queryStatusLabel = null;
        private Button queryCancelButton = null;
        private bool isQueryCancelled = false;
        ...
        public Form1()
        {
            ...
            this.Text = "SoilSensorTool_V1.4.0";

            tabControl.Alignment = TabAlignment.Left;
            tabControl.DrawMode = TabDrawMode.OwnerDrawFixed;
            tabControl.ItemSize = new Size(50, tabControl.Height / 5);
            tabControl.DrawItem += (s, e) =>
            {
                ...
                e.Graphics.DrawString(page.Text, tabControl.Font, Brushes.Black, rect, sf);
            };
            ...
            // 定时器
            timerCurrentTime = new System.Windows.Forms.Timer();
        ...
        private void CreateProgressForm()
        {
            ...
            progressForm.Size = new Size(400, 200);
            progressForm.Text = "数据查询进度";

            queryProgressBar = new ProgressBar();
            queryProgressBar.Location = new Point(20, 60);
            queryProgressBar.Size = new Size(340, 23);
            queryProgressBar.Minimum = 0;
            queryProgressBar.Maximum = 100;

            queryStatusLabel = new Label();
            queryStatusLabel.Location = new Point(20, 20);
            queryStatusLabel.Size = new Size(340, 30);
            queryStatusLabel.Text = "正在查询数据...";
            queryStatusLabel.Font = new Font("宋体", 12F, FontStyle.Bold);

            Label detailLabel = new Label();
            detailLabel.Location = new Point(20, 100);
            detailLabel.Size = new Size(340, 20);
            detailLabel.Text = "准备中...";
            detailLabel.Name = "detailLabel";

            queryCancelButton = new Button();
            queryCancelButton.Location = new Point(160, 130);
            queryCancelButton.Size = new Size(80, 30);
            queryCancelButton.Text = "取消";
            queryCancelButton.Font = new Font("宋体", 10F, FontStyle.Bold);
            queryCancelButton.Click += (s, e) =>
            {
                isQueryCancelled = true;
                if (dataQueryWorker != null && dataQueryWorker.IsBusy)
                {
                    dataQueryWorker.CancelAsync();
                }
                if (allDataQueryWorker != null && allDataQueryWorker.IsBusy)
                {
                    allDataQueryWorker.CancelAsync();
                }
                queryCancelButton.Enabled = false;
                queryCancelButton.Text = "正在取消...";
                queryStatusLabel.Text = "正在取消操作...";
            };

            progressForm.Controls.Add(queryProgressBar);
            progressForm.Controls.Add(queryStatusLabel);
            progressForm.Controls.Add(detailLabel);
            progressForm.Controls.Add(queryCancelButton);
        ...
        // 更新进度弹窗
        private void UpdateProgressForm(int percentage, string status, string detail)
        {
            if (progressForm != null && !progressForm.IsDisposed)
            {
                if (progressForm.InvokeRequired)
                {
                    progressForm.Invoke(new Action<int, string, string>(UpdateProgressForm), percentage, status, detail);
                    return;
                }

                if (queryProgressBar != null)
                {
                    queryProgressBar.Value = Math.Max(0, Math.Min(100, percentage));
                }

                if (queryStatusLabel != null)
                {
                    queryStatusLabel.Text = status ?? "处理中...";
                }

                Label detailLabel = progressForm.Controls.Find("detailLabel", false).FirstOrDefault() as Label;
                if (detailLabel != null)
                {
        ...
        private void CloseProgressForm()
        {
            if (progressForm != null && !progressForm.IsDisposed)
            {
                if (progressForm.InvokeRequired)
                {
                    progressForm.Invoke(new Action(CloseProgressForm));
                    return;
                }
                progressForm.Close();
                progressForm = null;
                ...
                if (cmd == CMD_QUERY_DEVICE_ADDR_REPLY)
                {
                    ...
                    if (content != null && content.Length >= 4)
                    {
                        uint deviceAddress = (uint)((content[0] << 24) |
                                                      ...
                                                      (content[2] << 8) |
                                                      content[3]);
                        Trace.WriteLine($"设备地址: {deviceAddress:X8}");
        ...
        private bool ReceiveMsg(out uint targetAddress, out uint sourceAddress, out byte cmd, out byte[] content, int timeout = 1000)
        {
            try
            {
                ...
                Trace.WriteLine($"收到命令: {cmd:X2}, 目标地址: {targetAddress:X8}, 源地址: {sourceAddress:X8}, 内容长度: {len}");
                ...
                if (content != null)
                {
                    ...
                    {
                        Trace.WriteLine("内容长度错误 " + rx_len + " " + len);
                        targetAddress = 0;
                        sourceAddress = 0;
                        cmd = 0;
                        content = null;
                ...
                {
                    Trace.WriteLine($"收到校验和: {checksum:X2}");
                    Trace.WriteLine("校验和错误");
                    targetAddress = 0;
                    sourceAddress = 0;
                    cmd = 0;
                    content = null;
        ...
        private void PrintHex(byte[] data)
        {
            ...
            Trace.Write("len " + data.Length + " bytes: ");

            foreach (byte b in data)
            {
                sb.Append(b.ToString("X2") + " ");
            }
            Trace.WriteLine(sb.ToString().Trim());
        }
        ...
        private void btQueryDevInfo_Click(object sender, EventArgs e)
        {
            ...
            Trace.WriteLine(comboBoxDevAdd.Text);
        ...
        private void btSetDevInfo_Click(object sender, EventArgs e)
        {
            ...
            Trace.WriteLine($"设置设备信息: SN={sn}, SensorNum={sensor_num}");
        ...
        // BackgroundWorker事件处理方法
        private void DataQueryWorker_DoWork(object sender, DoWorkEventArgs e)
        {
            BackgroundWorker worker = sender as BackgroundWorker;
            var parameters = e.Argument as dynamic;
            
            try
            {
                uint targetAddress = parameters.targetAddress;
                uint sourceAddr = parameters.sourceAddr;
                byte sensorIdx = parameters.sensorIdx;
                uint totalCount = parameters.totalCount;
                ...
                {
                    uint queryRecordCount = count > 25 ? 25u : count;
                    ...
                    bool receiveSuccess = ReceiveMsg(out responseTargetAddress, out responseSourceAddress, out cmd, out responseContent, 10000);

                    if (receiveSuccess && cmd == CMD_QUERY_COLLECTION_DATA_REPLY)
                    {
                        if (responseContent != null && responseContent.Length >= 10)
                        {
                            ...
                            for (int i = 0; i < recordCount; i++)
                            {
                                ...
                                string id = (sensorIdx + 1).ToString();
                    ...
                    int progress = (int)((totalCount - count) * 100 / totalCount);
                    worker.ReportProgress(progress, $"已处理 {processedRecords} 条记录，剩余 {count} 条");
                }
        ...
        private void DataQueryWorker_ProgressChanged(object sender, ProgressChangedEventArgs e)
        {
            UpdateProgressForm(e.ProgressPercentage, "正在查询数据...", e.UserState?.ToString());
        }

        private void DataQueryWorker_RunWorkerCompleted(object sender, RunWorkerCompletedEventArgs e)
        {
            // 禁用取消状态
            ...
            // 重新启用按钮
            btQuerySensorData.Enabled = true;
            
            if (e.Cancelled)
            {
                AddLogToTextBox("数据查询已取消");
                MessageBox.Show("数据查询已取消");
            }
            else if (e.Result is Exception)
            {
                Exception ex = e.Result as Exception;
                AddLogToTextBox($"数据查询失败: {ex.Message}");
                MessageBox.Show($"数据查询失败: {ex.Message}");
            }
            else
            {
                AddLogToTextBox("数据查询完成");
        ...
        private void btQuerySensorData_Click(object sender, EventArgs e)
        {
            ...
            if (textRecordCount.Text == "")
            { 
                MessageBox.Show("请查询记录数");
                return;
            }

            uint targetAddress = Convert.ToUInt32(comboBoxDevAdd.Text, 16);
            uint sourceAddr = ADDR_UPPER;
            byte sensorIdx = (byte)comboboxSensorIdx.SelectedIndex;
            uint totalCount = Convert.ToUInt32(textRecordCount.Text);
            ...
            // 禁用按钮
            btQuerySensorData.Enabled = false;
            ...
            // 创建BackgroundWorker
            dataQueryWorker = new BackgroundWorker();
            dataQueryWorker.WorkerReportsProgress = true;
            dataQueryWorker.WorkerSupportsCancellation = true;
            dataQueryWorker.DoWork += DataQueryWorker_DoWork;
            dataQueryWorker.ProgressChanged += DataQueryWorker_ProgressChanged;
            dataQueryWorker.RunWorkerCompleted += DataQueryWorker_RunWorkerCompleted;
            
            // 启动后台任务
            var parameters = new { targetAddress, sourceAddr, sensorIdx, totalCount };
            dataQueryWorker.RunWorkerAsync(parameters);
        }

        private void AllDataQueryWorker_DoWork(object sender, DoWorkEventArgs e)
        {
            BackgroundWorker worker = sender as BackgroundWorker;
            var parameters = e.Argument as dynamic;
            ...
            try
            {
                ...
                // 1. 查询设备信息获取传感器数量
                byte sensor_num = 0;
                byte[] queryDeviceInfo = LoadMsg(CMD_QUERY_DEVICE_INFO, targetAddress, sourceAddress, null, 0);
                PrintHex(queryDeviceInfo);
                SendHex(queryDeviceInfo);
                ...
                receiveSuccess = ReceiveMsg(out receivedTargetAddress, out receivedSourceAddress, out cmd, out content);

                if (receiveSuccess && cmd == CMD_QUERY_DEVICE_INFO_REPLY && content != null && content.Length >= 9)
                {
                ...
                worker.ReportProgress(5, $"设备共有 {sensor_num} 个传感器");

                // 2. 遍历所有传感器，获取其数据总量
                Dictionary<int, uint> sensorDataCounts = new Dictionary<int, uint>();
                uint totalRecordsToFetch = 0;
                ...
                for (int i = 0; i < sensor_num; i++)
                {
                    ...
                    byte[] query_time_count_content = new byte[] { (byte)i };
                    byte[] queryMsg = LoadMsg(CMD_QUERY_COLLECTION_START_TIME, targetAddress, sourceAddress, query_time_count_content, (byte)query_time_count_content.Length);
                    PrintHex(queryMsg);
                    SendHex(queryMsg);

                    receiveSuccess = ReceiveMsg(out receivedTargetAddress, out receivedSourceAddress, out cmd, out content);

                    if (receiveSuccess && cmd == CMD_QUERY_COLLECTION_START_TIME_REPLY && content != null && content.Length == 8)
                    {
                        current_sensor_count = (uint)(content[4] << 24 | content[5] << 16 | content[6] << 8 | content[7]);
                        sensorDataCounts.Add(i, current_sensor_count);
                        totalRecordsToFetch += current_sensor_count;
                    }
                    else
                    {
                        // 即使某个传感器查询失败，也继续查询下一个
                        worker.ReportProgress(0, $"查询传感器 {i + 1} 数据总量失败");
                    }
                }

                if (worker.CancellationPending) { e.Cancel = true; return; }
                worker.ReportProgress(10, $"总计需要获取 {totalRecordsToFetch} 条数据");

                // 3. 遍历所有传感器，分块获取数据
                uint processedRecords = 0;
                foreach (var sensorEntry in sensorDataCounts)
                {
                    int current_sensor_id = sensorEntry.Key;
                    uint current_sensor_total_count = sensorEntry.Value;
                    uint idx = current_sensor_total_count;
                    uint count = current_sensor_total_count;
                    ...
                    while (count > 0)
                    {
                        ...
                        byte[] queryMsg = LoadMsg(CMD_QUERY_COLLECTION_DATA, targetAddress, sourceAddress, query_data_content, (byte)query_data_content.Length);
                        PrintHex(queryMsg);
                        SendHex(queryMsg);

                        receiveSuccess = ReceiveMsg(out receivedTargetAddress, out receivedSourceAddress, out cmd, out content, 10000);

                        if (receiveSuccess && cmd == CMD_QUERY_COLLECTION_DATA_REPLY && content != null && content.Length >= 10)
                        {
                            int recordCountInBatch = content.Length / 10;
                            for (int i = 0; i < recordCountInBatch; i++)
                            {
                                uint timestamp = (uint)(content[i * 10] << 24 | content[i * 10 + 1] << 16 | content[i * 10 + 2] << 8 | content[i * 10 + 3]);
                                ushort tempture = (ushort)(content[i * 10 + 4] << 8 | content[i * 10 + 5]);
                                ushort humidity = (ushort)(content[i * 10 + 6] << 8 | content[i * 10 + 7]);
                                ushort conductivity = (ushort)(content[i * 10 + 8] << 8 | content[i * 10 + 9]);

                                string[] rowData = new string[] {
                                    DateTimeOffset.FromUnixTimeSeconds(timestamp).ToString("yyyy-MM-dd HH:mm:ss"),
                                    (current_sensor_id + 1).ToString(),
                                    (tempture / 10.0).ToString("F1"),
                                    (humidity / 10.0).ToString("F1"),
                                    (conductivity / 1.0).ToString("F2")
                                };

                                // 使用 UserState 传递行数据，并在UI线程上更新
                                worker.ReportProgress((int)(10 + (processedRecords * 90 / totalRecordsToFetch)), rowData);
                                processedRecords++;
                            }
                        }
                        else
                        {
                            // 即使某个传感器查询失败，也继续查询下一个
                             worker.ReportProgress(0, $"从传感器 {current_sensor_id + 1} 获取数据失败");
                        }

                        count -= queryRecordCount;
                        idx -= queryRecordCount;

                        int progress = (int)(10 + (processedRecords * 90 / totalRecordsToFetch));
                        worker.ReportProgress(progress, $"已处理 {processedRecords} / {totalRecordsToFetch} 条");
                    }
                }
            }
            catch (Exception ex)
        ...
        private void AllDataQueryWorker_ProgressChanged(object sender, ProgressChangedEventArgs e)
        {
            if (e.UserState is string[] rowData)
            {
                dataGridViewSensor.Rows.Add(rowData);
                // Optionally update a detailed status label if you have one
                string detail = $"已处理 {dataGridViewSensor.Rows.Count} 条...";
                UpdateProgressForm(e.ProgressPercentage, "正在查询所有数据...", detail);
            }
            else if (e.UserState is string statusMessage)
            {
                UpdateProgressForm(e.ProgressPercentage, "正在查询所有数据...", statusMessage);
            }
        }

        private void AllDataQueryWorker_RunWorkerCompleted(object sender, RunWorkerCompletedEventArgs e)
        {
            isQueryCancelled = false;
            CloseProgressForm();
            btQueryAllData.Enabled = true;
            ...
            {
                AddLogToTextBox("所有数据查询完成");
                MessageBox.Show("所有数据查询完成");
            }
        }
        ...
        private void btQueryAllData_Click(object sender, EventArgs e)
        {
            ...
            isQueryCancelled = false;
            btQueryAllData.Enabled = false;

            CreateProgressForm();
            progressForm.Show(this);

            allDataQueryWorker = new BackgroundWorker();
            allDataQueryWorker.WorkerReportsProgress = true;
            allDataQueryWorker.WorkerSupportsCancellation = true;
            allDataQueryWorker.DoWork += AllDataQueryWorker_DoWork;
            allDataQueryWorker.ProgressChanged += AllDataQueryWorker_ProgressChanged;
            allDataQueryWorker.RunWorkerCompleted += AllDataQueryWorker_RunWorkerCompleted;

            uint targetAddress = Convert.ToUInt32(comboBoxDevAdd.Text, 16);
            var parameters = new { targetAddress };
            allDataQueryWorker.RunWorkerAsync(parameters);
        }
        ...
        private void openUpgradeFileBt_Click(object sender, EventArgs e)
        {
            using (OpenFileDialog openFileDialog = new OpenFileDialog())
            {
                openFileDialog.Title = "选择升级文件";
                ...
                if (openFileDialog.ShowDialog() == DialogResult.OK)
                {
                    try
                    {
                        upgradeFileData = System.IO.File.ReadAllBytes(openFileDialog.FileName);
                        upgradeFileName = System.IO.Path.GetFileName(openFileDialog.FileName);

                        upgradeFileNameLb.Text = upgradeFileName;

                        MessageBox.Show($"文件加载成功!\n文件名: {upgradeFileName}\n文件大小: {upgradeFileData.Length} 字节");

                        AddLogToTextBox($"升级文件已加载: {upgradeFileName}, 大小: {upgradeFileData.Length} 字节");
                    }
                    catch (Exception ex)
                    {
                        MessageBox.Show($"读取文件失败: {ex.Message}");
                        AddLogToTextBox($"读取升级文件失败: {ex.Message}");
                        upgradeFileData = null;
                        upgradeFileName = null;
                    }
                }
            }
        }

        private void startUpgradeBt_Click(object sender, EventArgs e)
        {
            if (upgradeFileData == null)
            {
                MessageBox.Show("请先选择升级文件");
                return;
            }
            ...
            if (isUpgrading)
            {
                MessageBox.Show("升级正在进行中");
                return;
            }

            var result = MessageBox.Show(
                $"即将开始设备升级，文件大小: {upgradeFileData.Length} 字节\n升级过程中请勿断开连接！\n是否继续？",
                "设备升级确认",
                MessageBoxButtons.YesNo,
                MessageBoxIcon.Warning);
        ...
        {
            isUpgrading = true;
            startUpgradeBt.Enabled = false;
            stopUpgradeBt.Enabled = true;

            upgradeWorker = new BackgroundWorker();
        ...
        private void UpgradeWorker_DoWork(object sender, DoWorkEventArgs e)
        {
            ...
            try
            {
                ...
                this.Invoke(new Action(() =>
                {
                    targetAddress = Convert.ToUInt32(comboBoxDevAdd.Text, 16);
                }));

                int totalSize = upgradeFileData.Length;
                int currentPos = 0;

                // 计算需要擦除的块数（每块2048字节）
                int eraseBlocks = (totalSize + 2047) / 2048;

                AddLogToTextBox($"需要擦除 {eraseBlocks} 个块，总大小: {totalSize} 字节");

                // 第一步：擦除操作
                for (int blockIndex = 0; blockIndex < eraseBlocks; blockIndex++)
                {
                    if (worker.CancellationPending)
                    {
                        e.Cancel = true;
                    ...
                    uint eraseAddress = (uint)(blockIndex * 2048);
                    if (!SendEraseCommand(targetAddress, eraseAddress))
                    {
                        throw new Exception($"擦除块 {blockIndex} 失败");
                    }

                    int progress = (blockIndex + 1) * 30 / eraseBlocks; // 擦除占总进度的30%
                    worker.ReportProgress(progress, $"擦除进度: {blockIndex + 1}/{eraseBlocks}");

                    System.Threading.Thread.Sleep(100); // 给设备处理时间
                }

                AddLogToTextBox("擦除完成，开始写入数据...");
                ...
                while (currentPos < totalSize)
                {
                    ...
                    int writeSize = Math.Min(UPGRADE_DATA_MAX_LEN, totalSize - currentPos);
                    byte[] writeData = new byte[writeSize];
                    Array.Copy(upgradeFileData, currentPos, writeData, 0, writeSize);

                    if (!SendWriteCommand(targetAddress, (uint)currentPos, writeData))
                    {
                        throw new Exception($"写入数据失败，地址: 0x{currentPos:X8}, 长度: {writeSize} {writeData.Length}");
                    }

                    currentPos += writeSize;

                    // 写入占总进度的70%，从30%开始
                    int progress = 30 + (currentPos * 70 / totalSize);
                    worker.ReportProgress(progress, $"写入进度: {currentPos}/{totalSize} 字节");

                    System.Threading.Thread.Sleep(50); // 给设备处理时间
                }
                ...
                // 升级完成
                {
                    ...
                    if (ReceiveMsg(out responseTargetAddress, out responseSourceAddress, out cmd, out responseContent, 5000))
                    {
                        if (cmd == CMD_UPGRADE_DEVICE_REPLY && responseContent != null && responseContent.Length > 0)
                        {
                            if (responseContent[1] != 1)
                            {
                                throw new Exception("写入升级标志失败");
                            }
                        }
                    }
                    else
                    {
                        throw new Exception("写入升级标志无响应");
                    }
                }

                worker.ReportProgress(100, "升级完成");
            }
            catch (Exception ex)
            {
                e.Result = ex;
        ...
        private bool SendEraseCommand(uint targetAddress, uint eraseAddress)
        {
            try
            {
                byte[] content = new byte[5];
                content[0] = UPGRADE_CMD_ERASE;

                // 4字节地址
                content[1] = (byte)((eraseAddress >> 24) & 0xFF);
                content[2] = (byte)((eraseAddress >> 16) & 0xFF);
                content[3] = (byte)((eraseAddress >> 8) & 0xFF);
                content[4] = (byte)(eraseAddress & 0xFF);

                byte[] eraseMsg = LoadMsg(
                    CMD_UPGRADE_DEVICE,
                    targetAddress,
                    ADDR_ADMIN,
                    content,
            ...
            {
                AddLogToTextBox($"擦除命令发送失败: {ex.Message}");
                return false;
            }
        }

        private bool SendWriteCommand(uint targetAddress, uint writeAddress, byte[] data)
        {
            try
            {
                byte[] content = new byte[5 + data.Length];
                content[0] = UPGRADE_CMD_WRITE;

                // 4字节地址
                content[1] = (byte)((writeAddress >> 24) & 0xFF);
                content[2] = (byte)((writeAddress >> 16) & 0xFF);
                content[3] = (byte)((writeAddress >> 8) & 0xFF);
                content[4] = (byte)(writeAddress & 0xFF);

                // 数据
                Array.Copy(data, 0, content, 5, data.Length);

                byte[] writeMsg = LoadMsg(
                    CMD_UPGRADE_DEVICE,
                    targetAddress,
                    ADDR_ADMIN,
                    content,
            ...
            {
                AddLogToTextBox($"写入命令发送失败: {ex.Message}");
                return false;
            }
        }
        ...
        private void UpgradeWorker_ProgressChanged(object sender, ProgressChangedEventArgs e)
        {
            ...
            if (e.UserState != null)
            {
                AddLogToTextBox(e.UserState.ToString());
            }
        }
        ...
        {
            isUpgrading = false;
            startUpgradeBt.Enabled = true;
            stopUpgradeBt.Enabled = false;

            if (e.Cancelled)
            {
                AddLogToTextBox("升级已取消");
                MessageBox.Show("升级已取消");
            }
            else if (e.Result is Exception)
            {
                Exception ex = e.Result as Exception;
                AddLogToTextBox($"升级失败: {ex.Message}");
                MessageBox.Show($"升级失败: {ex.Message}");
            }
            else
            {
                AddLogToTextBox("设备升级成功完成！");
                MessageBox.Show("设备升级成功完成！");
            }
        ...
        private void stopUpgradeBt_Click(object sender, EventArgs e)
        {
            if (isUpgrading && upgradeWorker != null)
            {
                var result = MessageBox.Show(
                    "确定要停止升级吗？\n停止升级可能导致设备无法正常工作！",
                    "停止升级确认",
                ...
                if (result == DialogResult.Yes)
                {
                    ...
                    AddLogToTextBox("正在停止升级...");
                }
            }
        }

        private void queryOnenetBt_Click(object sender, EventArgs e)
        {
            if (!serialPort.IsOpen)
            {
                MessageBox.Show("请先打开串口");
            ...
            if (ReceiveMsg(out responseTargetAddress, out responseSourceAddress, out cmd, out responseContent))
            {
                if (cmd == CMD_QUERY_ONENET_REPLY)
                {
                    PrintHex(responseContent);
                    // 解析OneNet信息
                    string onenetProductId = "";
                    string onenetDeviceName = "";
                    string onenetProductKey = "";

                    int index = 0;
                    // 读取Product ID
                    while (index < responseContent.Length && responseContent[index] != 0)
                    {
                        onenetProductId += (char)responseContent[index];
                        index++;
                    }
                    ...
                    while (index < responseContent.Length && responseContent[index] != 0)
                    {
                        onenetDeviceName += (char)responseContent[index];
                        index++;
                    }
                    ...
                    while (index < responseContent.Length && responseContent[index] != 0)
                    {
                        onenetProductKey += (char)responseContent[index];
                        index++;
                    }

                    Trace.WriteLine($"Onenet Product ID: {onenetProductId}");
                    Trace.WriteLine($"Onenet Device Name: {onenetDeviceName}");
                    Trace.WriteLine($"Onenet Product Key: {onenetProductKey}");

                    productIdTextBox.Text = onenetProductId;
                    deviceNameTextBox.Text = onenetDeviceName;
                    productKeyTextBox.Text = onenetProductKey;
        ...
        private void setOnenetBt_Click(object sender, EventArgs e)
        {
            string onenetProductId = productIdTextBox.Text.Trim();
            string onenetDeviceName = deviceNameTextBox.Text.Trim();
            string onenetProductKey = productKeyTextBox.Text.Trim();

            if (string.IsNullOrEmpty(onenetProductId) ||
                string.IsNullOrEmpty(onenetDeviceName) ||
                string.IsNullOrEmpty(onenetProductKey))
            {
                MessageBox.Show("请填写完整的OneNet配置信息");
                return;
            }
            ...
            byte[] content = new byte[onenetProductId.Length + onenetDeviceName.Length + onenetProductKey.Length + 3];

            Array.Copy(Encoding.ASCII.GetBytes(onenetProductId), 0, content, 0, onenetProductId.Length);
            content[onenetProductId.Length] = 0; // 分隔符

            Array.Copy(Encoding.ASCII.GetBytes(onenetDeviceName), 0, content, onenetProductId.Length + 1, onenetDeviceName.Length);
            content[onenetProductId.Length + 1 + onenetDeviceName.Length] = 0; // 分隔符

            Array.Copy(Encoding.ASCII.GetBytes(onenetProductKey), 0, content, onenetProductId.Length + 1 + onenetDeviceName.Length + 1, onenetProductKey.Length);
            content[content.Length - 1] = 0; // 结束符
            ...
            if (ReceiveMsg(out responseTargetAddress, out responseSourceAddress, out cmd, out responseContent))
            {
                if (cmd == CMD_SET_ONENET_REPLY)
                {
                    PrintHex(responseContent);

                    if (responseContent[0] == 1)
                    {
                        MessageBox.Show("Onenet Info 设置成功");
                    }
                    else
                    {
                        MessageBox.Show("Onenet Info 设置失败");
                    }
                }
                else
                {
